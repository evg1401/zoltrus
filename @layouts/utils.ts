...
export const isNavLinkActive = (link: NavLink, router: Router) => {
  ...
  // })
  return matchedRoutes.some(route => (route.name === resolveRoutedName && Object.keys(router.currentRoute.value?.params ?? {}).length === 0) || (route.name === resolveRoutedName && JSON.stringify(router.currentRoute.value.params) === JSON.stringify(link.to?.params)) || route.meta.navActiveLink === resolveRoutedName)
}
